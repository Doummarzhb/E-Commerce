class Admin extends User {
    ...
    editProduct(products){
        if (!checkItemsAvailability(products)) {
            ...
            do {
                price = parseFloat(prompt(`Please enter the new price of the ${name} `));
            } while (!isNaN(price) && price < 0);
            image = prompt(`Please enter the new image URL of the ${name}`);
            do {
                quantity = parseInt(prompt(`Please enter the new quantity of the ${name}`));
    ...
    displayUsers(users) {
        if (!checkItemsAvailability(users)) {
            ...
            for (let user of users) {
                availabeUsers += `${user.username} and the email is ${user.email}\n`
            }
            alert(availabeUsers)
        }
    }
